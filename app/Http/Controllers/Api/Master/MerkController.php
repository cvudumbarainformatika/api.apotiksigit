...
class MerkController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$kode) {
            DB::select('call kode_merk(@nomor)');
            $nomor = DB::table('counter')->select('kode_merk')->first();
            $validated['kode'] = FormatingHelper::genKodeDinLength($nomor->kode_merk, 4, 'MRK');
        }

        $data = Merk::updateOrCreate(
            [
                'kode' => $validated['kode']
            ],
            $validated
